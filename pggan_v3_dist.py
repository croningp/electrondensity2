...
class ResBlockDown(Model):
    def __init__(self, num_channels, pooling='MaxPool3D'):
        ...
        self.conv_1x1 = Conv3D(num_channels, 1, padding='same',  kernel_initializer=tf.keras.initializers.Orthogonal())
        self.conv_3x3a = Conv3D(num_channels, 3, padding='same', kernel_initializer=tf.keras.initializers.Orthogonal())
        self.conv_3x3b = Conv3D(num_channels, 3, padding='same', kernel_initializer=tf.keras.initializers.Orthogonal())
...
class ConvSelfAttn(Model):
    def __init__(self,  attn_dim, output_dim):
        ...
        self.f_conv = Conv3D(attn_dim, 1,  kernel_initializer=tf.keras.initializers.Orthogonal())
        self.g_conv = Conv3D(attn_dim, 1,  kernel_initializer=tf.keras.initializers.Orthogonal())
        self.h_conv = Conv3D(attn_dim, 1,  kernel_initializer=tf.keras.initializers.Orthogonal())
        self.v_conv = Conv3D(output_dim, 1,  kernel_initializer=tf.keras.initializers.Orthogonal())
...
class CNNEncoder(Model):
    ...
    def __init__(self):
        ...
        self.activation = Activation('relu')
        self.dense = Dense(1024, activation='relu',  kernel_initializer=tf.keras.initializers.Orthogonal())
...
class CNNDecoder(Model):
    ...
    def __init__(self, use_batchnorm):
        ...
        self.resblockup_1 = ResBlockUp(256, use_batchnorm=use_batchnorm)
        self.resblockup_2 = ResBlockUp(128, use_batchnorm=use_batchnorm)
        self.resblockup_3 = ResBlockUp(64, use_batchnorm=use_batchnorm)
        self.resblockup_4 = ResBlockUp(32, use_batchnorm=use_batchnorm)
        ...
        self.dense = Dense(4096, activation='relu', kernel_initializer=tf.keras.initializers.Orthogonal())
        self.attn = ConvSelfAttn(attn_dim=32, output_dim=128)
...
class Generator(Model):
    def __init__(self, use_batchnorm, hidden_dim=1024):
        
        super(Generator, self).__init__()
        self.hidden_dim = hidden_dim
        self.decoder = CNNDecoder(use_batchnorm=use_batchnorm)
...
with mirrored_strategy.scope():
    ...
    generator = Generator(use_batchnorm=True)
    ...
    #g_checkpoint.restore('/media/extssd/jarek/models/gen.ckpt-50')
    #d_checkpoint.restore('/media/extssd/jarek/models/dis.ckpt-50')
...
def discrimator_train_step(density, batch_size=16, p_lambda=10):
    ...
    def step_fn(density):
        ...
        #loss = 0.5*tf.square(discriminator(density) - 1.0) + 0.5* tf.square(discriminator(generator(batch_size))+1)
            ...
            generated = generator(batch_size, training=False)
    ...
    per_replica_losses = mirrored_strategy.experimental_run_v2(step_fn, args=(density,))
    loss = mirrored_strategy.reduce(tf.distribute.ReduceOp.MEAN, per_replica_losses, axis=None)
    return loss
...
def generator_train_step(batch_size=16):
    ...
    def step_fn(batch_size=batch_size):
        ...
        #loss = tf.square(discriminator(generator(batch_size))-1)
            loss = discriminator(generator(batch_size, training=True))
    ...
    per_replica_losses = mirrored_strategy.experimental_run_v2(step_fn, args=())
    loss = mirrored_strategy.reduce(tf.distribute.ReduceOp.MEAN, per_replica_losses, axis=None)
    return loss
...
g_running_avg = 0.0
d_running_avg = 0.0
with mirrored_strategy.scope() as s:
    for j in tqdm.tqdm(range(100000)):
        ...
        with summary_writer.as_default():
            tf.summary.scalar('g_loss', gloss, step=counter)
            tf.summary.scalar('d_loss', dloss, step=counter)
        ...
        if counter % 10 == 0:
            ...
            g_losses = []
            d_losses = []
            generated_cubes = generator(8, training=False)
