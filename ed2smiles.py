...
class CustomCell(Layer):
    """
    Custom RNN cell for generating smiles 
    """
    ...
    def call(self, input, states, ):
        ...
        # run lstm1 using inputs and state from previous time step
...
class E2S(Layer):
    ...
    def call(self, densities, smiles):
        ...
        #smiles = tf.one_hot(smiles, depth=23, on_value=1.0, off_value=0.0)
        encoded_molecule = self.encoder(densities)
        self.rnn_cell.set_keys(encoded_molecule)
        #encoded_molecule = tf.expand_dims(encoded_molecule, axis=1)
        #encoded_molecule = tf.tile(encoded_molecule, [1, smiles.shape[1], 1]
...
checkpoint.restore('models\\ed2smiles_v3\\ed2smiles.ckpt-72')
...
def benchmark_model():
    ...
    for i in tqdm.tqdm(range(20)):
        ...
        #original_smiles_list__ += tokenizer.batch_decode_smiles(smiles_idxs.numpy())
        ...
        #return original_smiles, sampled_smiles, 
...
#    densities, _, smiles = valid_batch_density.__next__()
