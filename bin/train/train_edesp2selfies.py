##########################################################################################
#
# Trains the ED decorated with ESP to selfies transformer
#
##########################################################################################
...
RUN_FOLDER = 'logs/ed_esp2selfies/'
...
if mode == 'build':
    ...
    RUN_FOLDER += '2023-09-06/'  # fill with the right date
...
tfr = TFRecordLoader(path2tf, batch_size=64, train=True, properties=['electrostatic_potential', 'selfies'])
tfr_va = TFRecordLoader(path2va, batch_size=64, properties=['electrostatic_potential', 'selfies'])

# path to selfies tokenizer
